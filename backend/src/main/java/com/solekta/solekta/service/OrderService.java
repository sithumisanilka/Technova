...
public class OrderService {
    ...
    public OrderDTO createOrderFromCart(CheckoutRequest checkoutRequest) {
        ...
        payment.setMethod(checkoutRequest.getPaymentMethod());
        ...
        try {
            emailService.sendOrderConfirmationEmail(checkoutRequest.getEmail(), orderDTO);
    ...
    public OrderDTO updateOrderStatus(Long orderId, Order.OrderStatus status) {
        ...
        try {
            ...
            if (order.getCustomerEmail() != null && !order.getCustomerEmail().isEmpty()) {
                emailService.sendOrderStatusUpdateEmail(order.getCustomerEmail(), orderDTO, oldStatus.toString(),
                        status.toString());
                log.info("Order status update email sent for order {} to {}",
                        order.getOrderNumber(), order.getCustomerEmail());
            } else {
                log.warn("No customer email found for order {}, skipping status update email", order.getOrderNumber());
            }
        } catch (Exception e) {
